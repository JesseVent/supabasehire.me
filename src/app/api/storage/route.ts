  ...
  try {
    ...
    if (action === "list-buckets") {
      ...
      if (!res.ok) {
        ...
        return NextResponse.json({ error: `Storage API error (${res.status}): ${text}` }, { status: 502 });
      }
    ...
    if (action === "delete-file") {
      if (!bucket || !prefix) {
        return NextResponse.json({ error: "bucket and prefix (file path) are required for delete-file" }, { status: 400 });
      }
