...
import { type AgentChatMessage, useAgentStore } from '@/store/agent-store'
...
function ReflectionCard({
  ...
  const hasContent = reflection.evaluation || reflection.memory || reflection.nextGoal
      ...
      {open && (
        <div className="mt-1 text-[11px] text-muted-foreground space-y-1 pl-4 border-l border-border">
          {reflection.evaluation && (
            <p>
              <span className="text-foreground/70 font-medium">Eval:</span> {reflection.evaluation}
            </p>
          )}
          {reflection.memory && (
            <p>
              <span className="text-foreground/70 font-medium">Memory:</span> {reflection.memory}
            </p>
          )}
          {reflection.nextGoal && (
            <p>
              <span className="text-foreground/70 font-medium">Next:</span> {reflection.nextGoal}
            </p>
          )}
        </div>
      )}
      ...
      {message.toolCall && (
        <div className="mt-1.5 flex items-start gap-1.5 text-[11px] text-muted-foreground">
          ...
          <code className="break-all">
            {message.toolCall.name}({JSON.stringify(message.toolCall.input).slice(0, 200)})
          </code>
        </div>
      )}
...
export function AgentChatPanel() {
  const { sidebarOpen, setSidebarOpen, messages, clearMessages } = useAgentStore()
  const { status, activityText, isReady, error, execute, stop, answerQuestion } = useDevtoolAgent()
  ...
  const isWaitingForAnswer =
    lastMessage?.role === 'system' && lastMessage.content.startsWith('Agent asks:')
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-muted-foreground px-6">
                ...
                <p className="text-xs text-center leading-relaxed">
                  Ask me to inspect your Supabase database, check RLS policies, run queries, or
                  navigate the tool for you.
                </p>
