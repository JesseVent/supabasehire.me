...
function calculateNodeWidth(columns: ColumnInfo[]): number {
  ...
  const maxLen = Math.max(...columns.map((c) => `${c.column_name} ${c.data_type}`.length))
...
function getForceLayoutedElements(nodes: Node[], edges: Edge[]): { nodes: Node[]; edges: Edge[] } {
  ...
  forceSimulation(simNodes)
    ...
    .force(
      'collide',
      forceCollide<(typeof simNodes)[0]>()
    ...
    .force(
      'gx',
      forceX<(typeof simNodes)[0]>(0).strength((n) => (connectedIds.has(n.id) ? 0.04 : 0.2))
    )
    .force(
      'gy',
      forceY<(typeof simNodes)[0]>(0).strength((n) => (connectedIds.has(n.id) ? 0.04 : 0.2))
    )
    .alphaDecay(0.02)
    .stop()
    .tick(500)
...
function TableNode({ data }: { data: TableNodeData }) {
  ...
  const isSecureView = isView && securityInvoker === true

  const rlsNoPolicies = !isView && rlsEnabled && !rlsUnknown && rlsPoliciesCount === 0

  const borderColor = isView
    ? isSecureView
      ? 'border-brand/30'
      : 'border-red-500/40'
    : rlsUnknown
      ...
      : rlsNoPolicies
        ...
        : rlsEnabled
          ? 'border-brand/30'
          : 'border-red-400/30'
  const shadowColor = isView
    ? isSecureView
      ? 'shadow-black/10'
      : 'shadow-red-200/60 dark:shadow-red-900/40'
    : rlsUnknown
      ? 'shadow-amber-200/60 dark:shadow-amber-900/40'
      : rlsNoPolicies
        ? 'shadow-amber-200/60 dark:shadow-amber-900/40'
        : rlsEnabled
          ? 'shadow-black/10'
          : 'shadow-red-200/60 dark:shadow-red-900/40'
  const bgColor = isView
    ? isSecureView
      ? 'bg-background'
  ...
  return (
    ...
    // biome-ignore lint/a11y/useKeyWithClickEvents: ReactFlow handles canvas-wide keyboard interactions
    <div
      className={cn(
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          {isView ? (
            <Eye className="size-3.5 shrink-0 text-foreground/60" />
          ) : (
      ...
      <div className="px-3 py-1 bg-white dark:bg-background">
        {columns.map((col, index) => {
          ...
          const isPk =
            !isView &&
            (col.column_name === 'id' ||
              col.column_name === `${tableName}_id` ||
              col.column_default?.includes('nextval'))

          const typeBadgeClass = isView
            ? isSecureView
      ...
      >
        {isView
          ? isSecureView
            ? 'Secure View (security_invoker = true)'
            : 'Warning: SECURITY DEFINER (RLS bypass risk)'
          : rlsUnknown
            ? 'RLS status unknown'
            : rlsNoPolicies
              ? 'RLS on — no policies (all access denied)'
              : `${rlsPoliciesCount} polic${rlsPoliciesCount === 1 ? 'y' : 'ies'}`}
      </div>
...
function buildNodesAndEdges(
  ...
  tables.forEach((table, index) => {
    ...
    const rlsUnknown =
      (rls as Record<string, unknown> & { rlsUnknown?: boolean })?.rlsUnknown ?? false
...
function SchemaDiagramInner({
  ...
  const handleAutoLayout = useCallback(() => {
    const next: 'TB' | 'LR' | 'Force' =
      layoutMode === 'TB' ? 'LR' : layoutMode === 'LR' ? 'Force' : 'TB'
    setLayoutMode(next)
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: layout needs to re-run only when table list length changes
  useEffect(() => {
