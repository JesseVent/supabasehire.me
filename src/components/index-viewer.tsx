...
const DEMO_INDEXES: IndexInfo[] = [
  ...
  {
    tableName: 'posts',
    indexName: 'posts_created_at_idx',
    columns: ['created_at'],
    type: 'btree',
    unique: false,
    scans: 3200,
  ...
  {
    tableName: 'comments',
    indexName: 'comments_post_id_idx',
    columns: ['post_id'],
    type: 'btree',
    unique: false,
    scans: 7800,
  ...
  {
    tableName: 'audit_logs',
    indexName: 'audit_logs_created_at_idx',
    columns: ['created_at'],
    type: 'btree',
    unique: false,
    scans: 0,
...
function deriveUniqueFromName(indexName: string): boolean {
  ...
  return (
    lower.endsWith('_pkey') ||
    lower.endsWith('_key') ||
    lower.includes('_uniq') ||
    lower.includes('_unique')
  )
}
        ...
        <Card>
          <CardContent className="py-12 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1">
              Connect to a Supabase project or try Demo mode to view index data
            </p>
          </CardContent>
        </Card>
      )}
      ...
      {error && !isLoading && activeConnectionId && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            ...
            <Button variant="outline" size="sm" onClick={fetchIndexes} className="ml-3 h-7 text-xs">
            ...
            <Alert className="border-amber-200 bg-amber-50/50 dark:border-amber-800 dark:bg-amber-950/20">
              ...
              <AlertDescription className="text-amber-700 dark:text-amber-300">
                <span className="font-semibold">
                  {unusedIndexes.length} unused index{unusedIndexes.length !== 1 ? 'es' : ''}
                </span>{' '}
                detected (0 scans). These indexes may be candidates for removal to save storage and
                        ...
                        sortedIndexes.map((idx, i) => {
                          ...
                          return (
                            <TableRow
                              ...
                              className={cn(isUnused && 'bg-amber-50/50 dark:bg-amber-950/10')}
                            >
                              ...
                              <TableCell className="font-mono text-xs">{idx.size}</TableCell>
                            </TableRow>
                          )
                        })
                      )}
