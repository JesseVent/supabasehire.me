...
export const SpanCardToggle = ({ isExpanded, title, onToggleClick }: SpanCardToggleProps) => (
  <Collapsible.Trigger asChild>
    ...
    >
      {isExpanded ? (
        <ChevronDown aria-hidden="true" className="text-agentprism-muted-foreground size-3" />
      ) : (
        <ChevronRight aria-hidden="true" className="text-agentprism-muted-foreground size-3" />
      )}
    </button>
  </Collapsible.Trigger>
)
