...
import type { ServeManager } from './serveManager'
  ...
  async resolveDebugConfiguration(
    ...
    const isOurs =
      ...
      (config.port === cfg.port ||
        (typeof config.name === 'string' && /supabase|edge/i.test(config.name)))
    ...
    const resolved: EdgeAttachConfig = {
      ...
      localRoot:
        (config.localRoot as string | undefined) ?? path.join(workspaceRoot, cfg.functionsPath),
