...
export default {
  fetch: withSupabase({ auth: ['secret', 'publishable'] }, async (req) => {
    ...
    try {
      ...
      otlpSpans.push(
        makeSpan(traceId, s1Id, rootId, 'discover_tables', s1Start, s1End, {
      ...
      otlpSpans.push(
        makeSpan(traceId, s2Id, rootId, 'inspect_columns', s2Start, s2End, {
          'db.system': 'postgresql',
      ...
      steps.push({
        ...
        result: { table: targetTable, columns },
      })
      ...
      otlpSpans.push(
        makeSpan(traceId, s3Id, rootId, 'count_rows', s3Start, s3End, {
          'db.system': 'postgresql',
      ...
      steps.push({
        ...
        result: { table: targetTable, rowCount },
      })
    } finally {
      await client.end()
    }
