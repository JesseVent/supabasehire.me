...
import { discoverSkills, allReferenceIds } from "./lib/discover.ts";
...
if (!existsSync(targetFile)) {
	console.error(`No results found at ${targetFile}. Run \`pnpm eval${useNoisy ? " --noisy" : ""}\` first.`);
	process.exit(1);
}
...
function renderTable(results: EvalResult[], columns: string[]): string[] {
	...
	lines.push(
		`${"Prompt".padEnd(PROMPT_W)} ${"Category".padEnd(CAT_W)} ${headerCols}`,
	);
	...
	for (const cat of categories) {
		...
		for (const row of catRows) {
			...
			const cells = columns
				.map((c) => rpad(refSet.has(c) ? "✓" : "·", COL_W))
				.join("");
			lines.push(
				`${pad(row.prompt, PROMPT_W)} ${pad(cat, CAT_W)} ${cells}`,
			);
		}
...
function renderDiff() {
	if (!existsSync(RESULTS_FILE) || !existsSync(NOISY_FILE)) {
		console.error("Need both matrix.json and matrix-noisy.json. Run both `pnpm eval` and `pnpm eval --noisy`.");
		process.exit(1);
	}
	...
	for (const ref of allRefs) { cleanCounts.set(ref, 0); noisyCounts.set(ref, 0); }

	for (const r of clean.results) for (const ref of r.references) cleanCounts.set(ref, (cleanCounts.get(ref) ?? 0) + 1);
	for (const r of noisy.results) for (const ref of r.references) noisyCounts.set(ref, (noisyCounts.get(ref) ?? 0) + 1);
	...
	for (const { ref, c, n, delta } of diffs) {
		...
		const bar = delta > 0 ? "▲".repeat(Math.min(delta, 8)) : delta < 0 ? "▼".repeat(Math.min(-delta, 8)) : "·";
		const flag = delta !== 0 ? (Math.abs(delta) >= 5 ? " ⚠" : "") : "";
		lines.push(`${ref.padEnd(58)} ${cPct.padStart(6)} ${nPct.padStart(6)} ${dStr.padStart(6)}  ${bar}${flag}`);
	}

	const totalClean = [...cleanCounts.values()].reduce((a, b) => a + b, 0);
	const totalNoisy = [...noisyCounts.values()].reduce((a, b) => a + b, 0);
	lines.push("─".repeat(85));
	lines.push(`${"Total reference loads".padEnd(58)} ${String(totalClean).padStart(6)} ${String(totalNoisy).padStart(6)} ${String(totalNoisy - totalClean).padStart(6)}`);
	lines.push("");
...
if (writeMd) {
	...
	for (const [ref, n] of [...hitCounts.entries()].sort((a, b) => b[1] - a[1])) {
		mdLines.push(
			`| ${ref} | ${n} | ${Math.round((n / data.total_prompts) * 100)}% |`,
		);
	}
